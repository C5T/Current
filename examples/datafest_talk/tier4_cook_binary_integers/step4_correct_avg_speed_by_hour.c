...
struct PerHourCounter {
  int64_t total_miles_times_100;
  int64_t total_seconds;
};
  
struct PerHourCounter per_hour_counters[24];  // Zero-initialized as global. -- D.K.

void UpdateAverageSpeed(struct PerHourCounter* self, int64_t miles_times_100, int64_t seconds) {
  self->total_miles_times_100 += miles_times_100;
  self->total_seconds += seconds;
}

double ComputeAverageSpeed(const struct PerHourCounter* self) {
  return self->total_seconds ? ((60.0 * 60.0 * self->total_miles_times_100) / (self->total_seconds * 100)) : 0.0;
}
...
void Run(const struct IntegerRide* data, size_t n) {
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (data[i].trip_distance_times_100 > 0 && trip_duration_seconds > 0) {
        ...
        ++total_considered;
        UpdateAverageSpeed(&per_hour_counters[data[i].pickup.hour], data[i].trip_distance_times_100, trip_duration_seconds);
      }
    }
  }
  if (total_considered) {
