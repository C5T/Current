...
struct TCPImplTelnet {
  static string Run(const string& host, const uint16_t port, function<void(Connection&)> f) {
    Connection c(ClientSocket(host, port));
    f(c);
    const size_t max_size = 1000;
    vector<uint8_t> buffer(max_size);
    const size_t length = c.BlockingRead(&buffer[0], max_size);
    return string(buffer.begin(), buffer.begin() + length);
  }
  static string Run(const string& host, const uint16_t port, const string& message = "") {
    return Run(host, port, [&message](Connection& c) {
      c.BlockingWrite(message);
      c.SendEOF();
    });
  }
};
...
TYPED_TEST(TCPTest, ReceiveMessage) {
  ...
  EXPECT_EQ(
      "BOOM",
      TypeParam::Run("localhost",
                     FLAGS_port));  //(string("telnet localhost ") + to_string(FLAGS_port) + " 2>/dev/null"));
  t.join();
}

TYPED_TEST(TCPTest, ReceiveMessageOfTwoUInt16) {
  thread t([]() { Connection(Socket(FLAGS_port).Accept()).BlockingWrite(vector<uint16_t>{0x3031, 0x3233}); });
  // Note: This tests endianness as well -- D.K.
  //  EXPECT_EQ("1032", Telnet(string("telnet localhost ") + to_string(FLAGS_port) + " 2>/dev/null"));
  EXPECT_EQ("1032",
            TypeParam::Run(
                "localhost",
                FLAGS_port));  // Telnet(string("telnet localhost ") + to_string(FLAGS_port) + " 2>/dev/null"));
  t.join();
}
...
TYPED_TEST(TCPTest, EchoMessage) {
  ...
  EXPECT_EQ("ECHO: TEST OK", TypeParam::Run("localhost", FLAGS_port, "TEST OK"));
...
TYPED_TEST(TCPTest, EchoMessageOfTwoUInt16) {
  ...
  EXPECT_EQ("UINT16-s: 3252 3244", TypeParam::Run("localhost", FLAGS_port, "R2D2"));
...
TYPED_TEST(TCPTest, EchoTwoMessages) {
  ...
  EXPECT_EQ("ECHO2: FOO,BAR",
            TypeParam::Run("localhost", FLAGS_port, [](Connection& c) {
              c.BlockingWrite("FOO");
              c.BlockingWrite("BAR");
              c.SendEOF();
            }));
...
TYPED_TEST(TCPTest, PrematureMessageEndingException) {
  ...
  thread t([&big_struct]() {
    ...
    c.BlockingWrite("PART");
  });
  EXPECT_EQ("PART",
            TypeParam::Run("localhost", FLAGS_port, [](Connection& c) {
              c.BlockingWrite("FUUU");
              c.SendEOF();
            }));
